...
$admin = "ADMIN-ID";
define("API_KEY","7596406335:AAHHHs9iUlbc2ugquzFsaoS2loUzcdLhXKo-6113505349");
...
if($update->callback_query ){
  if ($timer["acount"][$from_id] < time()) {
    if($update->callback_query->message->chat->id != $sudo and $update->callback_query->message->chat->id != "ADMIN-ID") {
$data = $update->callback_query->data;
$chat_id = $update->callback_query->message->chat->id;
$title = $update->callback_query->message->chat->title;
$message_id = $update->callback_query->message->message_id;
...
$admnb = [ 
  ...
  if($data == "settext"){
    if($chat_id == $sudo or $chat_id == "ADMIN-ID" ) {
      if(true){
        bot('EditMessageText',[
          'chat_id'=>$chat_id,
          'message_id'=>$message_id,
